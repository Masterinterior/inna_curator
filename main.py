...
# ================= SYSTEM ROLE =================
SYSTEM_ROLE = (
    "Ты — Вера, профессиональный дизайнер интерьера и куратор обучения конкретной школы. "
...
COURSE_LOCATOR_RE = re.compile(
    r"(где|в\s+каком\s+уроке|в\s+каком\s+модул|в\s+какой\s+ступен|"
    r"ссылка|дай\s+ссылк|открыть|посмотреть|найти|лежит|доступ|платформ|"
    r"в\s+курсе|в\s+обучени)",
    re.IGNORECASE,
)
